...
import uk.gov.hmrc.platformstatusconsulfrontend.config.AppConfig
import uk.gov.hmrc.platformstatusconsulfrontend.connectors.{BackendConnector, InternetConnector}
...
class StatusChecker @Inject()(
  ...
  private def checkMongoConnection(dbUrl: String): Future[PlatformStatus] =
    ...
    val database   : MongoDatabase             = mongoClient.getDatabase("platform-status-consul-frontend")
