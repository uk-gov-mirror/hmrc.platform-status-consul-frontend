  ...
  "Internal auth feature switch" when:
    ...
    "enabled (default)" should:
      "require authentication for protected endpoints" in:
        ...
        val result = route(
          app,
          FakeRequest(GET, "/platform-status-consul/noise")
        ...
        verify(
          postRequestedFor(urlEqualTo("/internal-auth/auth"))
            .withRequestBody(containing("platform-status-consul-frontend"))
        )

        app.stop()
      end "require authentication for protected endpoints"

      "redirect to access denied when not authenticated" in:
        val app = GuiceApplicationBuilder()
          .configure(
            "microservice.services.internal-auth.host" -> wireMockHost,
            "microservice.services.internal-auth.port" -> wireMockPort,
        ...
        app.stop()
      end "redirect to access denied when not authenticated"
    end "enabled (default)"
    ...
    "disabled" should:
      ...
      "not require session token" in:
        ...
        val result = route(
          app,
          FakeRequest(GET, "/platform-status-consul/platformstatus")
        ).value
